# pyinstaller.exe --onefile --console --icon=icon.ico osutwitch.py --name OsuTwitch_v3_win64

# osu!twitch is on public release v3

# For research purposes only - Ashe Muller
...
rpc_client_id = '874687941350412298' 
RPC = Presence(rpc_client_id)  
RPC.connect() 
RPC.update(state="using osu!twitch version 3", large_image="osutwitch_hirez", buttons=[{"label": "Link To Github Download", "url": "https://github.com/MaleVTuber/osutwitch/releases/latest"}])
...
bot = commands.Bot(
    token="", #left out for privacy purposes
    client_id="", #also left out for privacy purposes
    nick='osu_twitchbot',
    prefix='!',
